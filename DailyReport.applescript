tell application "OmniFocus"
	tell front document
		set todayOffset to 0
		try
			set dialogResult to display dialog "Enter 'today' offset:" default answer todayOffset
			set todayOffset to text returned of dialogResult as number
		end try

		set yesterdayOffset to -1
		try
			set dialogResult to display dialog "Enter 'yesterday' offset from 'today:'" default answer yesterdayOffset
			set yesterdayOffset to text returned of dialogResult as number
		end try

		set today to (current date) + todayOffset * days
